...
Imgx=squeeze(max(Img,[],2));
Imgy=squeeze(max(Img,[],1));
Imgz=squeeze(max(Img,[],3));
y2z=size(Img,3)/size(Img,1);
ax_xy=axes('position',[0,0.4,0.8,0.8]), imshow(Imgz(:,:,S), [Rmin Rmax]);

x_pos=ax_xy.Position(2)-ax_xy.Position(4)*y2z;
y_pos=ax_xy.Position(1)+ax_xy.Position(3);
ax_xz=axes('position',[0,0.2,0.8,0.8*y2z]), imshow(Imgx(:,:,S)', [Rmin Rmax]);
ax_yz=axes('position',[y_pos,0.4,0.8*y2z,0.8]), imshow(Imgy(:,:,S), [Rmin Rmax]);
...
% -=< Slice slider callback function >=-
    function SliceSlider (hObj,event)
        ...
        set(get(ax_xy,'children'),'cdata',Imgz(:,:,S))
        set(get(ax_yz,'children'),'cdata',Imgx(:,:,S))
        set(get(ax_xz,'children'),'cdata',Imgy(:,:,S))
        caxis([Rmin Rmax])
        if sno > 1
