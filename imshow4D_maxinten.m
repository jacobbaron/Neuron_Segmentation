function  [ax_xyz,shand,stxthand]=imshow4D_maxinten( Img, f ,S)
%Img=cropZeros(Img);
% f=figure;
...
not_buttons_height=img_height+img_space;
not_buttons_width=img_width+img_space;
%xmargin=(fWidth-not_buttons_width)/2;
xy_pos=[img_space,img_space,img_width,img_height];
...
Imax=max(Img(:));
%xz_pos=[0,
ax_xyz=axes(f,'units','pixels','position',xy_pos); 
axis equal
img_size=size(Img);
imshow(zeros(img_size(1)+img_size(3)+1,...
                img_size(2)+img_size(3)+1),[Imin,Imax]);
ax_xyz.XTickLabel='';
ax_xyz.YTickLabel='';
%x_pos=ax_xyz.Position(2)-ax_xyz.Position(4)*y2z;
%y_pos=ax_xyz.Position(1)+ax_xyz.Position(3);
%xz_pos=[
...
Stxt_Pos = [50 65 uint16(FigPos(3)-100)+1 30];
...
if sno > 1
    ...
    stxthand = uicontrol('Parent',f,'Style', 'text','Position', Stxt_Pos,'String',sprintf('frame %d / %d',S, sno), 'BackgroundColor', [0.8 0.8 0.8], 'FontSize', SFntSz);
else
...
% set(get(ax_xyz,'Children'),'ButtonDownFcn', @mouseClick);
...
% -=< Slice slider callback function >=-
    function SliceSlider (hObj,event)
        ...
        set(get(ax_xyz,'children'),'cdata',Img_xy(:,:,S))
        %set(get(ax_yz,'children'),'cdata',Img_yz(:,:,S))
        %set(get(ax_xz,'children'),'cdata',Img_xz(:,:,S)')
        if sno > 1
            set(stxthand, 'String', sprintf('frame %d / %d',S, sno));
        else
