...
if (nargin < 1)
    ...
    if ~ishghandle(varargin{1})
        CleanGlobals;
        error(message('images:getrect:expectedHandle'));
    end
    ...
    otherwise
        CleanGlobals;
        error(message('images:getrect:expectedFigureOrAxesHandle'));
    end
end
...
if (ishghandle(GETRECT_H1))
    delete(GETRECT_H1);
end
if (ishghandle(GETRECT_H2))
    delete(GETRECT_H2);
end

% Restore the figure state
if (ishghandle(GETRECT_FIG))
   uirestore(state);
   
   if ishghandle(GETRECT_AX)
...
case 'trap'
    ...
    error(message('images:getrect:interruptedMouseSelection'));
...
global GETRECT_AX GETRECT_H1 GETRECT_H2
global GETRECT_PT1 GETRECT_TYPE
